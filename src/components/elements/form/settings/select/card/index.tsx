...
function CardSelectForm<T>({ value, options, gridProps, smallCard, largeCard, tablePaginationProps }: CardSelectFormProps<T>) {
	return (
		<Box my={2}>
			{value ?
				largeCard(value)
				: <>
					<Grid container spacing={2}>
						{options.map((option, i) =>
							<Grid {...gridProps} item key={i} display="flex" justifyContent="center">
								{smallCard(option)}
							</Grid>
						)}
					</Grid>
